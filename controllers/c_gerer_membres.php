 ...
 * BioGro
 * © GroSoft, 2017
 * Gestion des membres : contrôleur
...
if (isset($_GET["action"])) {
    ...
    $action = 'listerMembres';
}

if (isset($_REQUEST["id"])) {
    $id = strip_tags($_REQUEST["id"]);
    $leMembre = Membres::chargerMembreParId($id);
}
...
// définition des routes
    switch ($action) {
        case 'listerMembres' : {
                ...
                if ($lesMembres === PDO_EXCEPTION_VALUE) {
                    $msg = 'Une erreur s\'est produite, contactez votre administrateur';
                    Application::addNotification($msg, MSG_ERROR);
                    $nbMembres = 0;
                } else {
                    $nbMembres = count($lesMembres);
                }
                include 'views/admin/v_lister_membres.php';
            }
            break;
        case 'consulterMembre' : {
                if (isset($_REQUEST["id"])) {
                    $intIdMembre = $id;
                    $strLoginMembre = $leMembre->getLogin();
                    $strNomMembre = $leMembre->getNom();
                ...
                if ($hasErrors) {
                    header('location:index.php?uc=gererMembres&action=listerMembres');
                } else {
                    include 'views/admin/v_consulter_membre.php';
                }
            }
            break;
        case 'ajouterMembre' : {
                ...
                switch ($option) {
                    ...
                    case 'validerProduit' : {
                            ...
                            if (isset($_POST["cmdValider"])) {
                                ...
                                if (!empty($_POST["txtCodeProduit"]) and ! empty($_POST["txtNomProduit"]) and ! empty($_POST["txtPrixAchat"]) and ! empty($_POST["txtPrixVente"])
                                ) {
        ...
        case 'modifierProduit' : {
                ...
                switch ($option) {
                    case 'saisirProduit' : {
                        ...
                        break;
                    case 'validerProduit' : {
                            if (!empty($_REQUEST["txtNomProduit"]) and ! empty($_REQUEST["txtPrixAchat"]) and ! empty($_REQUEST["txtPrixVente"])
                            ) {
